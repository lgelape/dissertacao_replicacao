...
# Carregar a base com resultado total do estado de MG, para obter a base com os resultados do municipio
...
estado <- getTse("http://agencia.tse.jus.br/estatistica/sead/eleicoes/eleicoes2012/votosecao/vsec_1t_MG.zip")
...
lv <- read.csv2("https://raw.githubusercontent.com/lgelape/dissertacao_replicacao/master/Atributos_lv_BH2012.csv", fileEncoding = "Latin1")
...
names(lv)[1]<- "END_LV"
names(lv)[2]<- "NUM_SECAO"
names(lv)[3]<- "NUM_ZONA"
names(lv)[4]<- "CD_MUN_TSE"
names(lv)[5]<- "CD_MUN_IBGE"
names(lv)[6]<- "NM_MUN"
names(lv)[7]<- "NM_BAIRRO_TSE"
names(lv)[8]<- "NM_LV"
...
num_eleitos <- c(10123, 10200, 12345, 13130, 13200, 13333, 13500, 13670, 13677, 14450, 15200, 17369, 19007, 19193, 19456, 20107, 23384, 23500, 25678, 27000, 27789, 31333, 36000, 40001, 40100, 40123, 40190, 40321, 40777, 43333, 43690, 44101, 45000, 45107, 45555, 65600, 65653, 70040, 70413, 70700, 70777)
...
write.csv2(base_eleitos, "BH2012_eleitos.csv", row.names = F)
