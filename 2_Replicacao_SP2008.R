...
url <- "http://agencia.tse.jus.br/estatistica/sead/odsele/votacao_secao/votacao_secao_2008_SP.zip"
estado <- getTse(url)
...
# Selecionar somente as observacoes correspondentes a cidade de SP

cidade <- estado[estado$CODIGO_MUNICIPIO == 71072,]
...
lv <- read.csv2("https://raw.githubusercontent.com/lgelape/dissertacao_replicacao/master/Atributos_lv_SP2008.csv", fileEncoding = "Latin1")

# Renomeando variaveis, para evitar criacao de colunas desnecessarias, quando juntarmos as tabelas

lv$CD_OBJETO_ <- NULL # excluir a variavel que nao sera utilizada
...
names(lv)[3] <- "NUM_ZONA"
names(lv)[4] <- "NUM_SECAO"
names(lv)[5] <- "NM_BAIRRO_TSE"
names(lv)[6] <- "NUM_LV"
names(lv)[7] <- "NM_LV"
names(lv)[8] <- "CD_CEP"
names(lv)[14] <- "NM_MUN"
...
# agregando base por bairro (no caso de SP, por distrito, de acordo com o nome do distrito, pois o codigo, por vezes esta corrompido)

banco$NM_BAIRRO_TSE <- clean.accent(banco$NM_BAIRRO_TSE) #limpando o acento das duas variaveis, por precaucao
banco$NM_DISTRIT <- clean.accent(banco$NM_DISTRIT)

base_agr <- aggregate(QTDE_VOTOS ~  NM_DISTRIT + CD_GEOCODD + NM_MUN + CD_GEOCODM + CD_MUN_TSE + DATA_GERACAO + ANO_ELEICAO + NUM_TURNO + CODIGO_CARGO + DESCRICAO_CARGO + NUM_VOTAVEL, data = banco, sum)
...
# Criando uma variavel que faca a distincao entre vereadores eleitos (1) e vereadores nao-eleitos (2)

num_eleitos <- c(10123, 11611, 11624, 12345, 13100, 13110, 13123, 13133, 13222, 13644, 13650, 13696, 13700, 13800, 13900, 14000, 14200, 14444, 15622, 15651, 20012, 22123, 22222, 22611, 22678, 22888, 23000, 23140, 25000, 25005, 25012, 25200, 25250, 25400, 25699, 40123, 40401, 43010, 43543, 43666, 45000, 45133, 45157, 45177, 45194, 45222, 45321, 45444, 45633, 45654, 45678, 45695, 45699, 65000, 65123)
...
dominancia <- dom(base_vereadores, "NUM_VOTAVEL", "NM_DISTRIT", "QTDE_VOTOS")
...
hh <- hhi(base_vereadores, "NUM_VOTAVEL", "NM_DISTRIT", "QTDE_VOTOS")
...
indiceG <- G(base_vereadores, "NUM_VOTAVEL", "NM_DISTRIT", "QTDE_VOTOS")
...
write.csv2(base_eleitos, "SP2008_eleitos.csv", row.names = F)
