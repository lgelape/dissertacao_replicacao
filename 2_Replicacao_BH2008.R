...
# Carregar a base com resultado total do estado de MG por secao eleitoral, para obter a base com os resultados do municipio
...
estado <- getTse("http://agencia.tse.jus.br/estatistica/sead/odsele/votacao_secao/votacao_secao_2008_MG.zip")
...
# Selecionar somente as observacoes correspondentes a cidade de BH

cidade <- estado[estado$CODIGO_MUNICIPIO == 41238,]
rm(estado) # exclui o objeto dos resultados estaduais da rotina do R, pois nao o usaremos mais

# Carregar a base de locais de votacao, que sera unida com a de resultados eleitorais
# a partir das variaveis NUM_ZONA (n. da zona eleitoral), e NUM_SECAO (n. da secao eleitoral)

lv <- read.csv2("https://raw.githubusercontent.com/lgelape/dissertacao_replicacao/master/Atributos_lv_BH2008.csv", fileEncoding = "Latin1")
...
names(lv)[7] <- "NUM_LV"
names(lv)[8] <- "NM_LV"
names(lv)[9] <- "CD_MUN_IBGE"
names(lv)[10] <- "NM_MUN"
names(lv)[15] <- "NM_BAIRRO_IBGE"
...
banco$NM_BAIRRO_TSE <- clean.accent(banco$NM_BAIRRO_TSE) # limpando os acentos das duas variaveis
banco$NM_BAIRRO_IBGE <- clean.accent(banco$NM_BAIRRO_IBGE)

base_agr <- aggregate(QTDE_VOTOS ~  NM_BAIRRO_IBGE + CD_GEOCODB + NM_MUN + CD_MUN_IBGE + CD_MUN_TSE + DATA_GERACAO + ANO_ELEICAO + NUM_TURNO + CODIGO_CARGO + DESCRICAO_CARGO + NUM_VOTAVEL, data = banco, sum)
...
num_eleitos <- c(10123, 12456, 12610, 13040, 13135, 13200, 13444, 13670, 13677, 14111, 14450, 15000, 15100, 15128, 15190, 17258, 17369, 20107, 22123, 23384, 23400, 25678, 31031, 31333, 33051, 33250, 33456, 36001, 36123, 40123, 40654, 43610, 43690, 44100, 44200, 45000, 45100, 45107, 65888, 70070, 70999)
...
write.csv2(base_eleitos, "BH2008_eleitos.csv", row.names = F)
