...
estado <- getTse("http://agencia.tse.jus.br/estatistica/sead/eleicoes/eleicoes2012/votosecao/vsec_1t_RJ.zip")
...
lv <- read.csv2("https://raw.githubusercontent.com/lgelape/dissertacao_replicacao/master/Atributos_lv_RJ2012.csv", fileEncoding = "Latin1")
...
base_agr <- aggregate(QTDE_VOTOS ~  NM_BAIRRO_IBGE + CD_GEOCODB + NM_MUN + CD_GEOCODM + CD_MUN_TSE + DATA_GERACAO + ANO_ELEICAO + NUM_TURNO + CODIGO_CARGO + DESCRICAO_CARGO + NUM_VOTAVEL, data = banco, sum)
...
num_eleitos <- c(10123, 10789, 11015, 11111, 11120, 12345, 12456, 13000, 13210, 13333, 13444, 14014, 14123, 15001, 15015, 15040, 15070, 15101, 15105, 15123, 15150, 15159, 15500, 15633, 15688, 15800, 17017, 19123, 20100, 20126, 22002, 22193, 25111, 25622, 25888, 27027, 27777, 27999, 28281, 31031, 36500, 40044, 40369, 43123, 45245, 45620, 50000, 50111, 50123, 50555, 55622)
...
write.csv2(base_eleitos, "RJ2012_eleitos.csv", row.names = F)
